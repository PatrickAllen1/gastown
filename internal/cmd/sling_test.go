...
func TestResolveTargetCreateSpawnsPolecatShorthandWhenPaneMissing(t *testing.T) {
	...
	spawnPolecatForSling = func(rigName string, opts SlingSpawnOptions) (*SpawnedPolecatInfo, error) {
		...
		if got := slingSpawnPolecatName(opts); got != "toast" {
...
// slingSpawnPolecatName reads the named-target field when the production
// options carry it. Reflection keeps this regression runnable against the
// current-main base, which predates the named-target seam and therefore has no
// such field yet.
func slingSpawnPolecatName(opts SlingSpawnOptions) string {
	field := reflect.ValueOf(opts).FieldByName("PolecatName")
...
func TestResolveTargetNamedPolecatRequiresCreateForMissingCanonicalPath(t *testing.T) {
	...
	spawnPolecatForSling = func(rigName string, opts SlingSpawnOptions) (*SpawnedPolecatInfo, error) {
		...
		return &SpawnedPolecatInfo{
			RigName:     rigName,
			PolecatName: slingSpawnPolecatName(opts),
			ClonePath:   filepath.Join(townRoot, "gastown", "polecats", slingSpawnPolecatName(opts)),
		}, nil
	}
	...
	if len(calls) != 1 || !calls[0].Create || slingSpawnPolecatName(calls[0]) != "toast" {
...
func TestResolveTargetMixedCaseNamedPolecatUsesCanonicalIdentity(t *testing.T) {
	...
	spawnPolecatForSling = func(rigName string, opts SlingSpawnOptions) (*SpawnedPolecatInfo, error) {
		...
		return &SpawnedPolecatInfo{
			RigName:     rigName,
			PolecatName: slingSpawnPolecatName(opts),
			ClonePath:   filepath.Join(t.TempDir(), "gastown", "polecats", slingSpawnPolecatName(opts)),
		}, nil
	}
	...
	if gotRig != "gastown" || slingSpawnPolecatName(gotOpts) != "toast" {
...
func namedPolecatCurrentMainNamespace(t *testing.T) string {
	t.Helper()
	...
	return fmt.Sprintf("gt-h36-%d-%s", os.Getpid(), name)
}
...
func setupNamedPolecatCurrentMainFixture(t *testing.T, mode, cleanupStatus string) namedPolecatCurrentMainFixture {
	...
	townRoot := t.TempDir()
	namespace := namedPolecatCurrentMainNamespace(t)
	rigPath := filepath.Join(townRoot, namespace)
	...
	if err := os.WriteFile(filepath.Join(townRoot, ".beads", "routes.jsonl"), []byte(fmt.Sprintf(`{"prefix":"gt-","path":"%s/mayor/rig"}`+"\n", namespace)), 0644); err != nil {
	...
	clonePath := filepath.Join(rigPath, "polecats", "toast", namespace)
	...
	preservedAssignee := namespace + "/polecats/toast"
	defaultAgentID := "gt-" + namespace + "-polecat-toast"
...
case "$cmd" in
    ...
    case " $* " in
      ...
      *)
        if [ "${BD_NAMED_MODE:-idle}" = "active" ]; then
          printf '%%s\n' '[{"id":"gt-preserved-work","title":"preserved work","issue_type":"task","status":"hooked","assignee":"'"$preserved_assignee"'","description":""}]'
        else
          printf '%%s\n' '[]'
        fi
        ;;
  ...
  show)
    if [ "${id:-}" = "gt-preserved-work" ]; then
      ...
      agent_json "${id:-$default_agent_id}"
    fi
    ;;
  create)
    exit 1
...
exit 0
`, descPath, preservedAssignee, defaultAgentID, descPath, descPath)
	...
	r := &rig.Rig{Name: namespace, Path: rigPath}
	tm := tmux.NewTmuxWithSocket(namespace + "-tmux")
...
func TestSpawnNamedPolecatRetainsDurableProfileWithoutOverride(t *testing.T) {
	...
	if _, err := spawnNamedPolecatForSling(fixture.rig.Name, fixture.rig, fixture.manager, fixture.tmux, SlingSpawnOptions{
		PolecatName: "toast",
		HookBead:    "gt-next",
	}); err != nil {
		t.Fatalf("spawnNamedPolecatForSling: %v", err)
...
func TestSpawnNamedPolecatRejectsNonReusablePreservedWork(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := spawnNamedPolecatForSling(fixture.rig.Name, fixture.rig, fixture.manager, fixture.tmux, SlingSpawnOptions{
				PolecatName: "toast",
				HookBead:    "gt-next",
			})
			if err == nil {
